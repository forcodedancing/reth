...
use reth_db::models::parlia::{Snapshot, VoteAddress, CHECKPOINT_INTERVAL};
...
use reth_primitives::{
    system_contracts::get_upgrade_system_contracts, Address, BlockNumber, BlockWithSenders, Bytes,
    ChainSpec, Header, PruneModes, Receipt, Transaction, TransactionSigned, B256, BSC_MAINNET,
    U256,
};
    ...
    EvmConfig: ConfigureEvm,
{
        ...
        DB: Database<Error = ProviderError>,
    {
        ...
        for (sender, transaction) in block.transactions_with_sender() {
            ...
            self.patch_mainnet(&block.header, transaction, &mut state);
            self.patch_chapel(&block.header, transaction, &mut state);
    ...
    P: ParliaProvider,
{
    ...
    fn do_system_call_before_execution(
                ...
                .is_on_feynman_at_timestamp(header.timestamp, parent.timestamp)
        {
            ...
            output.validator_election_info =
                Some(self.parlia().unpack_data_into_validator_election_info(bz.as_ref()));
        }
